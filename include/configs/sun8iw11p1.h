...
#define CONFIG_SYS_PROMPT		"m2u# " /* "sunxi#" BPI */
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"bpim2u_mode=bpi-m2u\0" \
	"initrd_addr=44300000\0" \
	"kernel_filename=bpi/uImage\0" \
	"fdt_filename_prefix=bpi/m2u\0" \
	"fdt_filename_suffix=.dtb\0" \
	"initrd_filename=initrd.img\0" \
	"bootenv_filename=uEnv.txt\0" \
	"load_bootenv=" \
		"echo Loading bpi-m2u ${bootenv_filename} from ${load_addr} (${boot_part})...;" \
	...
	"load_dtb=" \
		...
		"fi; " \
		"echo Loading bpi-m2u ${fdt_filename} from ${fdt_addr} ...;" \
                "fatload mmc ${boot_part} ${fdt_addr} ${fdt_filename}; " \
		"fdt addr -c ${fdt_addr}; fdt resize\0" \
	"load_kernel=" \
		"echo Loading bpi-m2u-x ${kernel_filename} from ${kernel_addr} ...;" \
	...
	"load_initrd=" \
		...
		"echo Loading bpi-m2u ${initrd_filename} from ${initrd_addr} ...;" \
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"load_dtb=" \
		...
		"fdt addr -c ${fdt_addr}; fdt resize\0" \
	"load_kernel=" \
		"echo Loading bpi-m2u-emmc ${kernel_filename} from ${kernel_addr} ...;" \
                "fatload mmc ${boot_part} ${kernel_addr} ${kernel_filename}\0" \
	"boot_kernel=" \
                "echo bootm kernel:${kernel_addr} initrd:${initrd_addr} ...;" \
                "echo bootm initd_size:${initrd_size} fdt_addr:${fdt_addr} ...;" \
